...
namespace Knife.DeferredDecals
{
    ...
    public class DeferredDecalsSystem : MonoBehaviour
    {
        ...
        private Dictionary<Camera, CommandBuffer> m_CamerasEmission = new Dictionary<Camera, CommandBuffer>();
        ...
        public void Enable()
        {
            ...
            {
                DestroyImmediate(exclusionCamera);
            }
        ...
        public void Disable()
        {
            ...
            foreach(var camBuffer in m_CamerasEmission)
            {
                // key - camera
                // value - buffer
        ...
        public void Render(Camera camera)
        {
            ...
            // after lighting buffer (emission)
            CommandBuffer bufferAfterLighting = null;
            if (m_CamerasEmission.ContainsKey(cam))
            {
                bufferAfterLighting = m_CamerasEmission[cam];
                bufferAfterLighting.Clear();
            }
            else
            {
                bufferAfterLighting = new CommandBuffer();
                bufferAfterLighting.name = "Deferred decals";
                m_CamerasEmission[cam] = bufferAfterLighting;
                ...
                cam.AddCommandBuffer(CameraEvent.AfterLighting, bufferAfterLighting);
            }
            ...
            if (UseExclusionMask)
            {
                ...
                bufferAfterLighting.EnableShaderKeyword("EXCLUSIONMASK");
            ...
            else
            {
                ...
                bufferAfterLighting.DisableShaderKeyword("EXCLUSIONMASK");
            }
            ...
            else
            {
                if(isSceneViewCamera)
                {
                    ...
                    m_CamerasSceneview[cam] = bufferAfterLighting;
            ...
            // copy g-buffer base color into a temporary RT
            var baseColorID = Shader.PropertyToID("_BaseColorCopy");
            bufferAfterLighting.GetTemporaryRT(baseColorID, -1, -1, 0, FilterMode.Bilinear, RenderTextureFormat.ARGB32);
            bufferAfterLighting.Blit(emissionTexture, baseColorID);

            // copy g-buffer normals into a temporary RT
            bufferAfterLighting.GetTemporaryRT(normalsID, -1, -1);
            bufferAfterLighting.Blit(BuiltinRenderTextureType.GBuffer2, normalsID);
            ...
            foreach (var decalGroup in decalsGrouped)
            {
                ...
                if (decalGroup.Key.Instancing)
                {
                    ...
                    for (int i = 0; i < batchCount; i++)
                    {
                        ...
                        if (drawCount > 0)
                        {
                            instancedBlock.SetVectorArray("_Tint", tintArray);
                            instancedBlock.SetVectorArray("_UV", uvArray);
                            bufferBeforeLighting.DrawMeshInstanced(CubeMesh, 0, decalGroup.Key.Material, 3, matrices, drawCount, instancedBlock);
                        }
                    }
                }
                else
                {
                    foreach (var decal in decalsList)
                    {
                        ...
                        uv = decal.UV;
                        bufferBeforeLighting.SetGlobalVector("_NotInstancedUV", uv);
                        bufferBeforeLighting.DrawMesh(CubeMesh, decal.transform.localToWorldMatrix, decal.DecalMaterial, 0, 3);
                    }
                }
            }
            ...
            // render emission decals into three MRTs
            bufferAfterLighting.SetRenderTarget(BuiltinRenderTextureType.CameraTarget, BuiltinRenderTextureType.CameraTarget);
            foreach (var decalGroup in decalsGrouped)
            {
                var decalsList = decalGroup.Value;
                if (decalGroup.Key.Instancing)
                {
                    ...
                    for (int i = 0; i < batchCount; i++)
                    {
                        ...
                        if (drawCount > 0)
                        {
                            instancedBlock.SetVectorArray("_Tint", tintArray);
                            instancedBlock.SetVectorArray("_UV", uvArray);
                            bufferAfterLighting.DrawMeshInstanced(CubeMesh, 0, decalGroup.Key.Material, 1, matrices, drawCount, instancedBlock);
                        }
                    }
                }
                else
                {
                    foreach (var decal in decalsList)
                    {
                        ...
                        uv = decal.UV;
                        bufferBeforeLighting.SetGlobalVector("_NotInstancedUV", uv);
                        bufferAfterLighting.DrawMesh(CubeMesh, decal.transform.localToWorldMatrix, decal.DecalMaterial, 0, 1);
                    }
                }
            }
            /*foreach (var decal in sortedDecalsBySortingOrder)
            {
                ...
                //bufferAfterLighting.DrawMesh(CubeMesh, decal.transform.localToWorldMatrix, decal.m_Material, 0, 1); // emission pass - 1
            }*/
                        ...
                        if (go != null)
                        {
                            ...
                            if (decal != null)
                            {
                                ...
                                if (lastSelectedDecals.TryGetValue(decal, out time))
                                {
                                    ...
                                    selectionBuffer.DrawMesh(CubeMesh, decal.transform.localToWorldMatrix, decal.DecalMaterial, 0, 2); // selection pass - 2
                                }
            ...
            bufferAfterLighting.ReleaseTemporaryRT(normalsID);
            bufferAfterLighting.ReleaseTemporaryRT(baseColorID);

            if (selectionBuffer != null)
            {
                selectionBuffer.ReleaseTemporaryRT(baseColorID);
