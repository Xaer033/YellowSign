	...
	SubShader
	{
		Pass // before lighting (diffuse and ambient light)
		{
			...
			void frag(v2f i, out float4 outDiffuse : COLOR0, out float4 outNormal : COLOR1, out float4 outEmission : COLOR2)
			{
				...
				outEmission = float4(col.rgb, col.a);
		...
		Pass // after lighting (emission)
		{
			...
			float4 frag(v2f i) : SV_TARGET
			{
				...
				fixed4 col = tex2D (_MainTex, i.uv) * _Color * _NotInstancedColor;
				col.a *= blendByNormal;
				col *= UNITY_ACCESS_INSTANCED_PROP(DecalProps, _Tint);
				float blendTerrains = 1;
				#if TERRAIN_DECAL && !NO_TERRAIN
				...
				fixed4 finalEmission = lerp(currentEmission, currentEmission + emission, col.a);

				return float4(finalEmission.rgb, col.a);
			}
			ENDCG
		}
		...
		Pass// Editor Only - SELECTION
		{
			...
			float4 frag(v2f i) : SV_TARGET
			{
				...
				selectionColor = CalcSelectionColor(i.uv, alpha, float2(0.00002 / notLinearDepth / scaleLength, 0.00002 / notLinearDepth / scaleLength)) * 2 * SelectionTime;
		...
		Pass // before lighting (spec blend)
		{
			...
			void frag(v2f i, out float4 outSpecular : COLOR0, out float4 outSmoothness : COLOR1)
			{
				...
				float4 col = tex2D (_MainTex, i.uv) * _Color * _NotInstancedColor;
				col.a *= blendByNormal;
				col *= UNITY_ACCESS_INSTANCED_PROP(DecalProps, _Tint);
				float blendTerrains = 1;
				#if TERRAIN_DECAL && !NO_TERRAIN
				...
				spec.a *= _Smoothness * col.a;

				outSpecular = float4(spec.rgb, col.a);
				outSmoothness = float4(spec.a, 0, 0, col.a);
			}
			ENDCG
		}
